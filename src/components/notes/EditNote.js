...
export default function EditNote({match}) {
    ...
    useEffect(() =>{
        const getNote = async () =>{
            ...
            if(match.params.id){
                const res = await axios.get(`https://notes-api-qbrw.onrender.com/api/notes/${match.params.id}`, {
    ...
    const editNote = async e => {
        ...
        try {
            ...
            if(token){
                ...
                await axios.put(`https://notes-api-qbrw.onrender.com/api/notes/${id}`, newNote, {
                    headers: {Authorization: token}
                })
